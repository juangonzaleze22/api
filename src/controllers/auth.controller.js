...
export const register = async (req, res) => {
    ...
    if (usernameDB){
         res.status(401).json({message: 'Ya existe este usuario'});
    }else if (emailDB){
        res.status(401).json({message: 'Ya existe este email'});
    }else{
...
export const login = async (req, res) => {
    ...
    if (!userFound) res.status(401).json({message: "Este usuario no existe"});
